...
Page({
  ...
  onLoad(option) {
    ...
    wx.showLoading();
    wx.request({
      url: app.httpUrl + '/yysXcxUserCenter/initGdclmx.x', // 该url是自己的服务地址，实现的功能是服务端拿到authcode去开放平台进行token验证
        data: {
          id: option.id,
          type:option.type
        },
  ...
  tsjycl:function(){
    ...
    console.log(param);
    wx.request({
      url: app.httpUrl + '/yysXcxUserCenter/goPtcl.x', // 该url是自己的服务地址，实现的功能是服务端拿到authcode去开放平台进行token验证
        data: param,
        success: (re) => {
          var a;
          if(re.data.status == '1'){
